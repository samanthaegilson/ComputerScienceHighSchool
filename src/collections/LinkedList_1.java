 ...
 * (dynamic) type (ADT) and useful methods. A linked list is a collection of
 * data objects "linked together" and where each object maintains a reference (a
 * "pointer") to the next object in the list. This is different from an array
 * where we have one identifier referencing all the items. This is useful
 * because each object (each spot) is connected to the next spot like a chain.
 * If we also use each "spot" to store data, we have a structure like an array
 * that stores multiple data items, but where we can add and remove "spots" from
 * the chain. It could be "visualized" as:
 *
 * +------+ +------+ +------+ +------+ null <---- | | <---- | | <---- | | <---- |      |
 *             | NODE |       | NODE |       | NODE |       | NODE |
 *             |      |----> | |----> | |----> | |----> null +------+ +------+ +------+ +------+
...
public class LinkedList_1<T> {
    ...
    /**
     * The number of nodes in the list, cannot be changed outside the class, so
     * it is an immutable encapsulated property
     */
    private int length;
     ...
     * immutable encapsulated property of the class) - no mutator method will be
     * written (so 'length' remains a read-only property from outside the class)
     *
    ...
    /**
     * Inserts (adds) data (generic type) into the back (tail, end, etc) of this
     * list
     *
    ...
    public boolean addAfter(T data, int index) {
        ...
        if (index == length - 1) {
            return addBack(data);    // Add to the end 
        }
    ...
    public boolean addBefore(T data, int index) {
        ...
        if (index == 0) {
            return addFront(data);          // Add to the head 
        }
        Node<T> node = new Node<>(data);                // Create node object
        Node current = getNode(index);                  // Get to index spot
    ...
    public T removeFront() {
        ...
        if (length == 1) {
            finalize();            // One node list, wipe list
        } else {
     ...
     */
    public LinkedList_1(LinkedList<T> list) {
        fromLinkedList(list);
    }
}
